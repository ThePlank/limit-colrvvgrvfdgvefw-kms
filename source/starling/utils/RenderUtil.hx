...
class RenderUtil {
    ...
    public static function clear(rgb:UInt=0, alpha:Float=0.0,depth:Float=1.0, stencil:UInt=0):Void {
    ...
    public static function getTextureLookupFlags(format:String, mipMapping:Bool,repeat:Bool=false,smoothing:String="bilinear"):String {
    ...
    /* Returns a bit field uniquely describing texture format and premultiplied alpha,
       ...
       useful when overriding the <code>programVariantName</code> method of custom effects.
    ...
    */
    ...
    // Calls <code>setSamplerStateAt</code> at the current context, converting the given parameters to their low level counterparts.
    ...
    /* Creates an AGAL source string with a <code>tex</code> operation, including an options list with the appropriate format flag.
       ...
       @param tempReg    if 'resultReg' is the output register and PMA conversion is done, a temporary register is needed.

       @return the AGAL source code, line break(s) included.
    */
    public static function createAGALTexOperation(resultReg:String, uvReg:String, sampler:Int, texture:Texture,convertToPmaIfRequired:Bool=true, tempReg:String="ft0"):String {
    ...
    /* Requests a context3D object from the given Stage3D object.
       ...
       @param profile If you know exactly which 'Context3DProfile' you want to use, simply pass a String with that profile.
       ...
       profile automatically. This will try all known Stage3D profiles, beginning with the most powerful.</p>
    */
    public static function requestContext3D(stage3D:Stage3D, renderMode:String, profile:Dynamic):Void {
        ...
        var executeFunc:Function->Array<Dynamic>->Void = SystemUtil.isDesktop ? execute : SystemUtil.executeWhenApplicationIsActive;
